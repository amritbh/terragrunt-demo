...
4. Fill out the required parameters:
   ...
   * **Environment:** Select `dev`, `uat`, `prod`, or **`All Environments`**.
   * **Confirmation:** You must type exactly `DESTROY` in all caps to unlock the safety mechanism.
5. Click **Run workflow**.

> **Note on "All Projects" & "All Environments":** Selecting "All Projects (Everything)" or "All Environments" will safely iterate through the corresponding folders and tear down each one sequentially. If you select "All Environments", the pipeline will enforce the `prod` GitHub Environment approval gate to ensure production cannot be destroyed without explicit human approval.
...
### What about the S3 State Bucket?

By design, running the manual teardown workflow **will not** destroy the `s3-terraform-terragrunt-state` bucket or the DynamoDB lock table. 

These resources are not part of your infrastructure modules (like VPC or EKS); they are the remote state backend automatically generated by Terragrunt. 

**Best Practice:** Leave the S3 bucket and DynamoDB table intact after a teardown. They cost virtually nothing to maintain ($0.0000X/month) and keeping them ensures that Terragrunt can seamlessly re-deploy the environment in the future without needing to re-initialize a brand new remote state backend.

> **Note:** If you are permanently decommissioning the entire project forever and will never deploy it again, you must delete the S3 bucket and DynamoDB table manually via the AWS Console after all environments have been successfully torn down.
